...
def test_apply_plan_creates_expected_output(tmp_path: Path) -> None:
    ...
    assert (setup_result.root_dir / "02_scripts" / "preparation" / "holo_amber_prepare.in").exists()
    assert (
        setup_result.root_dir / "02_scripts" / "simulation" / "05_post_processing" / "03_merge_production.cpptraj"
    ).exists()
    assert (
        setup_result.root_dir / "02_scripts" / "simulation" / "06_analysis" / "03_radius_of_gyration.cpptraj"
    ).exists()
...
def test_render_prepare_files_uses_variant_pdb_id_references_without_download(tmp_path: Path) -> None:
    ...
    apo_path = (tmp_path / "variant-id-demo" / "02_scripts" / "preparation" / "apo_amber_prepare.in").as_posix()
    holo_path = (tmp_path / "variant-id-demo" / "02_scripts" / "preparation" / "holo_amber_prepare.in").as_posix()
