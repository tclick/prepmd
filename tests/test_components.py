...
def test_cli_prepare(tmp_path: Path) -> None:
    ...
    assert result.exit_code == 0
    assert (tmp_path / "prep" / "02_scripts" / "preparation" / "apo_replica_001_gromacs_prepare.in").exists()
...
def test_cli_prepare_with_config_and_cli_overrides(tmp_path: Path) -> None:
    ...
    assert result.exit_code == 0
    assert (tmp_path / "from-config" / "02_scripts" / "preparation" / "apo_replica_001_gromacs_prepare.in").exists()
...
def test_cli_prepare_offline_uses_cached_pdb_without_network(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert result.exit_code == 0
    assert (tmp_path / "prep-offline" / "02_scripts" / "preparation" / "apo_replica_001_amber_prepare.in").exists()
...
def test_cli_prepare_supports_apo_holo_pdb_ids(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    apo_prepare = tmp_path / "prep-variant-ids" / "02_scripts" / "preparation" / "apo_replica_001_amber_prepare.in"
    holo_prepare = tmp_path / "prep-variant-ids" / "02_scripts" / "preparation" / "holo_replica_001_amber_prepare.in"
...
def test_cli_prepare_with_orthorhombic_box_dimensions(tmp_path: Path) -> None:
    ...
    assert result.exit_code == 0
    prep_file = tmp_path / "prep-ortho" / "02_scripts" / "preparation" / "apo_replica_001_gromacs_prepare.in"
...
def test_cli_prepare_adds_ions_and_neutralization_for_amber(tmp_path: Path) -> None:
    ...
    assert result.exit_code == 0
    prep_file = tmp_path / "prep-ions" / "02_scripts" / "preparation" / "apo_replica_001_amber_prepare.in"
