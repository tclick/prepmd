...
class StructureBuilder(SimulationPlan):
    ...
    def __init__(self, config: ProjectConfig) -> None:
        ...
        self._variant_pdb_files = self._resolve_variant_pdb_files()
    ...
    def _resolve_shared_pdb_file(self) -> str | None:
        ...
        downloaded = PDBHandler(
            cache_dir=cache_dir,
            offline=protein.offline,
            structure_format=protein.structure_format,
        ).get_or_download(protein.pdb_id)
        return str(downloaded)

    def _resolve_variant_pdb_files(self) -> dict[str, str | None]:
        protein = self.config.protein
        cache_dir = Path(protein.pdb_cache_dir) if protein.pdb_cache_dir is not None else None
        handler = PDBHandler(
            cache_dir=cache_dir,
        ...
        for variant in sorted(protein.variants):
            ...
            if remote_id is None:
                variant_files[variant] = None
                continue
            variant_files[variant] = str(handler.get_or_download(remote_id))
