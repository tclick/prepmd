...
def _amber_scripts() -> dict[str, str]:
    return {
        "02_scripts/post_processing/01_strip_waters.cpptraj": _normalize_script(
            """
            parm ../system.prmtop
        ...
        "02_scripts/post_processing/02_center.cpptraj": _normalize_script(
            """
            parm ../system.prmtop
        ...
        "02_scripts/post_processing/03_merge_production.cpptraj": _normalize_script(
            """
            parm ../system.prmtop
            trajin ../04_production/run_001/production.nc
            trajin ../04_production/run_002/production.nc
        ...
        "02_scripts/post_processing/04_visualize_merged.vmd.tcl": _normalize_script(
            """
        ...
        "02_scripts/analysis/01_rmsd.cpptraj": _normalize_script(
            """
            parm ../system.prmtop
            trajin ../post_processing/centered_protein.nc
            rms first @CA out rmsd_ca.dat
            run
            """
        ),
        "02_scripts/analysis/02_rmsf.cpptraj": _normalize_script(
            """
            parm ../system.prmtop
            trajin ../post_processing/centered_protein.nc
        ...
        "02_scripts/analysis/03_radius_of_gyration.cpptraj": _normalize_script(
            """
            parm ../system.prmtop
            trajin ../post_processing/centered_protein.nc
            radgyr :* out rgyr.dat
        ...
        "02_scripts/analysis/04_hbond.cpptraj": _normalize_script(
            """
            parm ../system.prmtop
            trajin ../post_processing/centered_protein.nc
        ...
        "02_scripts/analysis/05_sasa.cpptraj": _normalize_script(
            """
            parm ../system.prmtop
            trajin ../post_processing/centered_protein.nc
...
def _gromacs_scripts() -> dict[str, str]:
    return {
        "02_scripts/post_processing/01_strip_waters.sh": _normalize_script(
            """
            #!/usr/bin/env bash
            set -euo pipefail
        ...
        "02_scripts/post_processing/02_center.sh": _normalize_script(
            """
            #!/usr/bin/env bash
            set -euo pipefail
        ...
        "02_scripts/post_processing/03_merge_production.sh": _normalize_script(
            """
            #!/usr/bin/env bash
            set -euo pipefail
            gmx trjcat -f \
        ...
        "02_scripts/analysis/01_rmsd.sh": _normalize_script(
            """
            #!/usr/bin/env bash
            set -euo pipefail
            printf '%s\\n%s\\n' Backbone Backbone | gmx rms \
              -s ../topol.tpr \
              -f ../post_processing/centered_protein.xtc \
              -o rmsd_backbone.xvg
            """
        ),
        "02_scripts/analysis/02_rmsf.sh": _normalize_script(
            """
            #!/usr/bin/env bash
            set -euo pipefail
            echo C-alpha | gmx rmsf -s ../topol.tpr -f ../post_processing/centered_protein.xtc -res -o rmsf_ca.xvg
            """
        ),
        "02_scripts/analysis/03_radius_of_gyration.sh": _normalize_script(
            """
            #!/usr/bin/env bash
            set -euo pipefail
            echo Protein | gmx gyrate -s ../topol.tpr -f ../post_processing/centered_protein.xtc -o rgyr.xvg
            """
        ),
        "02_scripts/analysis/04_hbond.sh": _normalize_script(
            """
            #!/usr/bin/env bash
            set -euo pipefail
        ...
        "02_scripts/analysis/05_sasa.sh": _normalize_script(
            """
            #!/usr/bin/env bash
            set -euo pipefail
            echo Protein | gmx sasa -s ../topol.tpr -f ../post_processing/centered_protein.xtc -o sasa_total.xvg
            """
        ),
    }
...
def _charmm_family_scripts() -> dict[str, str]:
    return {
        "02_scripts/post_processing/01_strip_waters.vmd.tcl": _normalize_script(
            """
        ...
        "02_scripts/post_processing/02_center.vmd.tcl": _normalize_script(
            """
        ...
        "02_scripts/analysis/01_rmsd.vmd.tcl": _normalize_script(
            """
            mol new ../system.psf type psf waitfor all
            mol addfile ../post_processing/centered_protein.dcd type dcd waitfor all
        ...
        "02_scripts/analysis/02_rmsf.vmd.tcl": _normalize_script(
            """
            mol new ../system.psf type psf waitfor all
            mol addfile ../post_processing/centered_protein.dcd type dcd waitfor all
        ...
        "02_scripts/analysis/03_radius_of_gyration.vmd.tcl": _normalize_script(
            """
            mol new ../system.psf type psf waitfor all
            mol addfile ../post_processing/centered_protein.dcd type dcd waitfor all
        ...
        "02_scripts/analysis/04_hbond.vmd.tcl": _normalize_script(
            """
            mol new ../system.psf type psf waitfor all
            mol addfile ../post_processing/centered_protein.dcd type dcd waitfor all
        ...
        "02_scripts/analysis/05_sasa.vmd.tcl": _normalize_script(
            """
            mol new ../system.psf type psf waitfor all
            mol addfile ../post_processing/centered_protein.dcd type dcd waitfor all
...
def _openmm_scripts() -> dict[str, str]:
    return {
        "02_scripts/post_processing/01_strip_waters.py": _normalize_script(
            """
            import mdtraj as md
        ...
        "02_scripts/post_processing/02_center.py": _normalize_script(
            """
            import mdtraj as md
        ...
        "02_scripts/post_processing/03_merge_production.py": _normalize_script(
            """
            import mdtraj as md
        ...
        "02_scripts/analysis/01_rmsd.py": _normalize_script(
            """
            import mdtraj as md
            import numpy as np

            traj = md.load("../post_processing/centered_protein.dcd", top="../system.pdb")
            ca = traj.topology.select("protein and name CA")
            rmsd = md.rmsd(traj, traj, atom_indices=ca, frame=0)
            np.savetxt("rmsd_ca.dat", rmsd)
            """
        ),
        "02_scripts/analysis/02_rmsf.py": _normalize_script(
            """
            import mdtraj as md
            import numpy as np
        ...
        "02_scripts/analysis/03_radius_of_gyration.py": _normalize_script(
            """
            import mdtraj as md
            import numpy as np
        ...
        "02_scripts/analysis/04_hbond.py": _normalize_script(
            """
            import mdtraj as md
            import numpy as np
        ...
        "02_scripts/analysis/05_sasa.py": _normalize_script(
            """
            import mdtraj as md
            import numpy as np
